...
START_TEST(from_decimal_to_int_15) {
  ...
  int check = 214748364, result = 0;  // (int)(INT_MAX / 10^1)
  int code = s21_from_decimal_to_int(a, &result);
  ck_assert_int_eq(result, check);
  ck_assert_int_eq(code, 0);
}
...
START_TEST(from_decimal_to_int_16) {
  ...
  int check = 0, result = 0;  // (int)(INT_MAX / 10^28)
  int code = s21_from_decimal_to_int(a, &result);
  ck_assert_int_eq(result, check);
  ck_assert_int_eq(code, 0);
}
